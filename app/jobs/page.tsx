            ...
            {QUICK_FILTERS.map(f => (
              <button key={f.value} onClick={() => setQuickFilter(f.value)}
                className={`px-2.5 py-1 rounded text-[12px] font-medium transition-colors ${
                  quickFilter === f.value ? 'bg-forest-800 text-white' : 'text-gray-600 hover:bg-stone-200'
                }`}>
                {f.label}
              </button>
            ))}
              ...
              <div className="flex justify-center items-center gap-1 mt-8">
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)}
                    className={`w-8 h-8 flex items-center justify-center rounded text-[13px] font-semibold transition-colors ${page === i + 1 ? 'bg-forest-800 text-white' : 'bg-white border border-stone-300 text-gray-700 hover:bg-stone-50'}`}>
                    {i + 1}
                  </button>
...
function LowDataFill({ createHref }: { createHref: string }) {
  return (
    <div className="mt-8 space-y-5">
      <section className="bg-forest-50/60 border border-forest-200/60 rounded-lg p-5 flex items-center gap-4 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <p className="text-[10px] font-semibold tracking-wider text-forest-700 uppercase">Cộng đồng Nhân Cơ</p>
        ...
        <Link href={createHref}
          className="bg-forest-800 hover:bg-forest-900 text-white text-[13px] font-semibold px-4 py-2 rounded transition-colors">
          + Đăng ngay
        </Link>
      </section>
      <section className="bg-amber-50/60 border border-amber-200/60 rounded-lg p-4 flex gap-3">
        ...
        <div>
          <p className="text-[13px] font-bold text-ink mb-1">Tìm việc an toàn</p>
        ...
        <div className="relative bg-stone-100 overflow-hidden" style={{ aspectRatio: '4/3' }}>
          {imgUrl ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-1"
              style={{ background: isEmployer ? 'linear-gradient(135deg,#241C15,#065f46)' : 'linear-gradient(135deg,#0c4a6e,#0369a1)' }}>
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            ...
            {!item.isVip && isNew && (
              <span className="bg-white/95 text-forest-700 border border-forest-200 text-[10px] font-bold px-1.5 py-0.5 rounded">MỚI</span>
            )}
          </div>
          {item.type && (
            <span className={`absolute top-2 right-2 text-[10px] font-semibold px-1.5 py-0.5 rounded ${isEmployer ? 'bg-forest-700 text-white' : 'bg-sky-600 text-white'}`}>
              {isEmployer ? 'Tuyển dụng' : 'Tìm việc'}
            </span>
          )}
        </div>
        <div className="p-3">
          <h4 className="text-[13.5px] font-medium text-gray-900 line-clamp-2 leading-snug min-h-[2.5em] mb-1.5 group-hover:text-forest-700 transition-colors">
            {item.title}
          </h4>
          {item.salary ? (
            <p className="text-[14px] font-bold text-forest-700 mb-2">{item.salary}</p>
          ) : (
          ...
          <div className="flex items-center gap-2 text-[11px] text-gray-500 mb-2">
            ...
            {typeof item._distanceKm === 'number' && (
              <span className="flex items-center gap-0.5 text-forest-700 font-semibold flex-shrink-0">
      ...
      <Link href={`/jobs/${item.id}`}
        ...
        <div className="relative w-32 sm:w-40 flex-shrink-0 bg-stone-100 overflow-hidden" style={{ aspectRatio: '4/3' }}>
          ...
          <div className="absolute top-1.5 left-1.5 flex flex-col gap-1">
            ...
            {!item.isVip && isNew && <span className="bg-white/95 text-forest-700 border border-forest-200 text-[10px] font-bold px-1.5 py-0.5 rounded">MỚI</span>}
          </div>
        </div>
        <div className="flex-1 min-w-0 py-2.5 pr-10">
          <div className="flex items-start gap-2 mb-0.5">
            <h4 className="flex-1 text-[14px] font-medium text-gray-900 line-clamp-2 leading-snug group-hover:text-forest-700 transition-colors">
              {item.title}
            </h4>
            {item.type && (
              <span className={`flex-shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded ${isEmployer ? 'bg-forest-100 text-forest-800' : 'bg-sky-100 text-sky-800'}`}>
                {isEmployer ? 'Tuyển dụng' : 'Tìm việc'}
              </span>
            )}
          </div>
          {item.salary ? (
            <p className="text-[14px] font-bold text-forest-700 mt-1">{item.salary}</p>
          ) : (
          ...
          <div className="flex items-center gap-3 text-[11px] text-gray-500 mt-2 flex-wrap">
            ...
            {typeof item._distanceKm === 'number' && (
              <span className="flex items-center gap-0.5 text-forest-700 font-semibold">
                <i className="ri-navigation-fill"></i>{formatDistance(item._distanceKm)}
              </span>
            )}
