...
export default function ChatPage() {
  ...
  async function loadMessages() {
    try {
      ...
      setMsgs(data.data || []);
      // Lấy info người kia từ conversations list
      const convs = await messages.getConversations() as any[];
  ...
  function setupSocket(uid: string | null) {
    const token = localStorage.getItem('accessToken');
  ...
  function sendMsg(content: string, type = 'TEXT', fileUrl?: string) {
    if (!socketRef.current?.connected) return;
    socketRef.current.emit('send_message', { conversationId, content, type, fileUrl });
  }
  ...
  async function handleBlock() {
    ...
    if (!confirm(`Chặn ${otherUser.fullName || otherUser.username}? Bạn sẽ không nhận tin nhắn từ họ.`)) return;
    try {
              ...
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} gap-2`}>
                ...
                <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                  {msg.type === 'IMAGE' && msg.fileUrl ? (
                    <img src={msg.fileUrl} alt="ảnh" className={`rounded-2xl max-w-full max-h-60 object-cover cursor-pointer ${isMe ? 'rounded-br-sm' : 'rounded-bl-sm'}`}
          ...
          {blocked ? (
            ...
            <form onSubmit={handleSend} className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => fileInputRef.current?.click()}
                ...
                {uploadingImg ? <div className="w-4 h-4 border-2 border-green-600 border-t-transparent rounded-full animate-spin"></div>
