...
const SUB_CATEGORIES = [
  ...
  { value: 'Chó', name: 'Chó' },
  { value: 'Mèo', name: 'Mèo' },
  { value: 'Gia cầm', name: 'Gia cầm' },
  { value: 'Gia súc', name: 'Gia súc' },
  { value: 'Cá cảnh', name: 'Cá cảnh' },
  { value: 'Chim', name: 'Chim' },
];
...
const POPULAR_SEARCHES = ['Chó corgi', 'Mèo anh lông ngắn', 'Gà thả đồi', 'Heo rừng', 'Cá koi', 'Bò sữa', 'Dê núi', 'Vịt xiêm'];
...
function VatNuoiInner() {
  ...
  async function loadProducts(searchQuery?: string) {
    ...
    try {
      ...
      const params: any = { category: 'VAT_NUOI', page, limit: 12, sortBy };
      ...
      <header className="border-y-2 border-ink bg-paper">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-5 lg:py-6">
          <div className="flex items-end justify-between gap-3 mb-4">
            <div>
              <nav className="text-[11px] text-ink-faint mb-1 flex items-center gap-1">
                ...
                <span className="text-ink-soft">Vật nuôi</span>
              </nav>
              <h1 className="text-xl sm:text-2xl font-bold text-ink leading-tight">
                {search ? `Kết quả "${search}"` : 'Mua bán vật nuôi tại Nhân Cơ'}
              </h1>
              <p className="text-[12px] text-ink-faint mt-0.5">
                <b className="text-ink">{fmt(total)}</b> tin đang rao · Đắk Nông
              </p>
            </div>
          ...
          <form onSubmit={handleSearch} className="flex gap-2 mb-3">
            ...
            <button type="submit" className="bg-ink hover:bg-black text-white px-4 py-2 rounded-md text-[13px] font-semibold transition-colors whitespace-nowrap">
              Tìm
            </button>
          </form>
            ...
            {SUB_CATEGORIES.map(cat => (
              <button key={cat.value}
                ...
                className={`flex-shrink-0 px-3 py-1 rounded-full text-[12px] font-medium whitespace-nowrap transition-colors ${
                  subCat === cat.value
                    ? 'bg-ink text-white'
                    : 'bg-surface border border-line text-ink-soft hover:border-ink/40'
                }`}>
                {cat.name}
              </button>
            ))}
            ...
            {QUICK_FILTERS.map(f => (
              <button key={f.value} onClick={() => setQuickFilter(f.value)}
                className={`px-2.5 py-1 rounded text-[12px] font-medium transition-colors ${
                  quickFilter === f.value ? 'bg-amber-800 text-white' : 'text-gray-600 hover:bg-stone-200'
                }`}>
                {f.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 ml-auto">
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded text-[12px] font-medium transition-colors ${
                userLoc ? 'bg-amber-100 text-amber-800 hover:bg-amber-200' : 'text-gray-600 hover:bg-stone-200'
              } disabled:opacity-50`}>
            ...
            <select value={sortBy} onChange={e => { setSortBy(e.target.value); setPage(1); }}
              className="text-[12px] border border-stone-300 rounded px-2 py-1 focus:outline-none focus:border-amber-700 bg-white font-medium text-gray-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
          <main className="min-w-0">
            {loading ? (
              ...
              <EmptyState
                ...
                onClearSearch={search || subCat ? () => { setSearch(''); setSubCat(''); router.replace('/vat-nuoi'); } : undefined}
              />
            ) : view === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {filtered.map(p => <ItemCard key={p.id} item={p} onDeleted={id => setItems(prev => prev.filter(x => x.id !== id))} accentColor="amber" />)}
              </div>
            ) : (
              <div className="space-y-2">
                {filtered.map(p => <ItemListRow key={p.id} item={p} onDeleted={id => setItems(prev => prev.filter(x => x.id !== id))} accentColor="amber" />)}
              </div>
            )}

            {lowData && filtered.length > 0 && <LowDataFill createHref="/products/create?category=VAT_NUOI" accentColor="amber" />}
              ...
              <div className="flex justify-center items-center gap-1 mt-8">
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)}
                    className={`w-8 h-8 flex items-center justify-center rounded text-[13px] font-semibold transition-colors ${page === i + 1 ? 'bg-amber-800 text-white' : 'bg-white border border-stone-300 text-gray-700 hover:bg-stone-50'}`}>
                    {i + 1}
                  </button>
          ...
          <div className="hidden lg:block">
            <CategorySidebar
              ...
              searchHref={(q) => `/vat-nuoi?search=${encodeURIComponent(q)}`}
              itemHref={(item) => `/products/${item.id}`}
              postHref="/products/create?category=VAT_NUOI"
              postLabel="Đăng tin vật nuôi"
              showAd={false}
            />
          </div>
        </div>
...
function LowDataFill({ createHref, accentColor }: { createHref: string; accentColor: string }) {
  const accent = accentColor === 'amber' ? 'amber' : 'indigo';
  return (
    <div className="mt-8 space-y-5">
      <section className={`bg-${accent}-50/60 border border-${accent}-200/60 rounded-lg p-5 flex items-center gap-4 flex-wrap`}>
        <div className="flex-1 min-w-[200px]">
          <p className={`text-[10px] font-semibold tracking-wider text-${accent}-700 uppercase`}>Cộng đồng Nhân Cơ</p>
          <h3 className="text-[15px] font-bold text-gray-900 mt-0.5 mb-1">Đăng tin đầu tiên trong khu vực của bạn</h3>
          <p className="text-[12.5px] text-gray-600 leading-relaxed">
            Bà con tại Nhân Cơ, Đắk Nông đang tìm mua vật nuôi. Đăng ngay để bán nhanh, gần nhà.
          </p>
        </div>
        <Link href={createHref}
          className={`bg-${accent}-800 hover:bg-${accent}-900 text-white text-[13px] font-semibold px-4 py-2 rounded transition-colors`}>
          + Đăng tin ngay
        </Link>
      </section>
      <section className="bg-amber-50/60 border border-amber-200/60 rounded-lg p-4 flex gap-3">
        ...
        <div>
          <p className="text-[13px] font-bold text-ink mb-1">Mua bán an toàn</p>
          <p className="text-[12px] text-ink/80 leading-relaxed">
            Gặp trực tiếp ở nơi đông người · Kiểm tra kỹ vật nuôi trước khi trả tiền · Không chuyển khoản trước.
        ...
        <div className="relative bg-stone-100 overflow-hidden" style={{ aspectRatio: '4/3' }}>
          {imgUrl ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-1">
              <i className="ri-bear-smile-line text-3xl text-stone-300"></i>
          ...
          {item.isVip ? (
            ...
            <span className={`absolute top-2 left-2 bg-white/95 text-${accent} text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200`}>
              MỚI
            </span>
          )}
        </div>
        <div className="p-3">
          <h4 className={`text-[13.5px] font-medium text-gray-900 line-clamp-2 leading-snug min-h-[2.5em] mb-1.5 group-hover:text-${accent} transition-colors`}>
            {item.title}
          </h4>
          ...
          <div className="flex items-center gap-2 text-[11px] text-gray-500 mb-2">
            ...
            {typeof item._distanceKm === 'number' && (
              <span className={`flex items-center gap-0.5 text-${accent} font-semibold flex-shrink-0`}>
      ...
      <Link href={`/products/${item.id}`}
        ...
        <div className="relative w-32 sm:w-40 flex-shrink-0 bg-stone-100" style={{ aspectRatio: '4/3' }}>
          ...
          {item.isVip ? (
            ...
            <span className={`absolute top-1.5 left-1.5 bg-white/95 text-${accent} text-[10px] font-bold px-1.5 py-0.5 rounded border border-amber-200`}>MỚI</span>
          )}
        </div>
        <div className="flex-1 min-w-0 py-2.5 pr-10">
          <h4 className={`text-[14px] font-medium text-gray-900 line-clamp-2 leading-snug group-hover:text-${accent} transition-colors`}>
            {item.title}
          </h4>
